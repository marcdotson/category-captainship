...
Create a loop that creates a synthetic control for each store in each retailer. Collect the p values from each synthetic control and create a distribution of p values across stores by retailer. PROBLEM IS THAT IT IS NOT RECOGNIZING ANY VARIABLES IN DC DATA. 
```{r}
...
for (i in seq_along(ret_codes)) {
  msynth_pvals <- c()
  msynth_effects <- c()
  t_stores <- dc_data %>% filter(retailer_code==ret_codes[i]) %>% as.numeric(retailer_code)
  c_stores <- dc_data %>% filter(retailer_code!=ret_codes[i]) %>% mutate(type=0) %>% as.numeric(retailer_code)
