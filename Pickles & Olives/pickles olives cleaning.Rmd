...
Unzip file, then read in files for `2011` using a loop which reads in a file if it is a `2011 movement` file and binds
each of the separate files into one.
```{r}
...
Read in and combine `2012 movement` files.
```{r}
move_12 <- tibble()
...
Read in and combine `2013 movement` files.
```{r}
move_13 <- tibble()
...
`F`, which denotes food, and for the 4 states where supervalue operates: North Dakota (`ND`), Minnesota (`MN`), 
Missouri (`MO`), and D.C. (`DC`). Call the merged and filtered file for `2011`, `full_11`, with similar names for the
other years.
```{r}
full_11 <- move_11 %>%
...
variable, and `sales` from multiplying the number of units sold `units` and the `price`.
```{r}
...
Create `tbls` containing each distinct store code (`store_id_11/12/13`). `anti_join` those three `tbls` to get 6
`tbls` containing just the stores that only show up in 1 of the 2 years ex: `in_11_not_12`. `anti_join` those with
the full data to remove stores that don't show up in the full time period in the data. Create a new variable 
`manuf_name` from the first three characters of the brand description. Remove unnecessary files from memory.
```{r}
...
Create vector `stores` that contains each distinct `store_code_uc` in the full movement file. Define a vector `retail_switchers` that will contain the codes for every store that switches retail codes. The loop finds these codes and stores them in `retail_switchers`.
```{r}
...
Filter `full_11_12_13` for observations where `retailer_code` is not equal to `NA` and save as `tbl retail_no_NA`. 
Filter `full_11_12_13` for observations where `retailer_code` is `NA` and impute `parent_code` to replace those 
missing values, then recombine with the observations that originally had a `retailer_code`.
```{r}
...
Save the full data as `pickles_olives_clean.RData`. Then filter for each state and save those data sets as well.
```{r}
save(full_11_12_13, file = 'pickles_olives_clean.RData')
...
save(mn_data, file = 'mn_pickles_olives_clean.RData')
save(nd_data, file = 'nd_pickles_olives_clean.RData')
save(mo_data, file = 'mo_pickles_olives_clean.RData')
save(dc_data, file = 'dc_pickles_olives_clean.RData')
```
