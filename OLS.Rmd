...
Assign munfacturer for each brand.Aggregate sales for all other stores in 842.
...
dc_data %>% group_by(store_code_uc, manuf_name) %>% summarize(sales=sum(sales)) 

treat <- dc_data %>% filter(retailer_code==842) %>% group_by(store_code_uc, manuf_name) %>% summarize(sales=sum(sales))

control <- dc_data %>% filter(retailer_code==32, 90,111) %>% group_by(store_code_uc, manuf_name) %>% summarize(sales=sum(sales))

```

Find store codes in 842, for all other store codes, take average, combine other and 842 into data frame 
