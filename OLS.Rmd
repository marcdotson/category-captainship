...
Assign munfacturer for each brand. Aggregate sales for all other stores not in 842. Divide all sales by average sales over entire pre and post period. 
...
ag_dc <- dc_data %>% group_by(store_code_uc, manuf_name, week_end) %>% summarize(sales=sum(sales)) 

treat <- ag_dc %>% filter(store_code_uc==treated_stores) %>% mutate(scaled_sales=sales/mean(sales))

control <- ag_dc %>% filter(store_code_uc!=treated_stores) %>% group_by(manuf_name, week_end) %>%
  summarize(sales=sum(sales)) %>% mutate(scaled_sales=sales/mean(sales), store_code_uc=00000)

ols_data <- treat %>% bind_rows(control) %>% mutate(treated=if_else(store_code_uc==treated_stores, 1, 0),
                                                    post=if_else(week_end > "2012-07-16", 1, 0))

```

Run OLS regression. Plot graph showing sales over time. 

```{r fig.height=7, fig.width=6}

ols_data %>% lm(scaled_sales~treated + post + I(treated*post), data=.) %>% summary()
...
ggplot(ols_data, aes(week_end, scaled_sales, col = factor(store_code_uc))) + geom_line() + 
  geom_smooth(aes(week_end, scaled_sales), data = pre_cc, method = 'lm', se = FALSE) + 
  geom_smooth(aes(week_end, scaled_sales), data = post_cc, method = 'lm', se = FALSE) + 
  geom_vline(xintercept = as.numeric(as.Date("2012-07-16"))) + 
ggtitle("Scaled Sales for treated store and aggregate") + theme(legend.position="bottom") 
