---
title: "Missouri Spread and Jam Graph Analysis"
...
Load data.
```{r}
load('mo_spreads_jams_clean.RData')
```
Create tbl `main_manuf_sales` which contains movement data for the brands ownedby Smucker's, Welch's, and private label. Create a new variable `actual_manuf` which denotes whether Smucker's, Welch's, or private label own a particular brand. This allows for sales across different brands to be aggregated later to sales by manufacturer.
```{r}
...
```
Create 'retailer_weekly', which is filtered for retailers in Missouri, grouped by retailer code, brand_name, and week_end, creating weekly sales and average prices by manufacturer within retailer. 'ret_codes' contains each unique retailer code. The 'for' loop goes over each unique retailer code and graphs the weekly sales by manufacturer for each retailer code. It also plots the average weekly price by manufacturer by retailer. It places these graphs next to each other for easy comparison. There is a verticle line at the 
