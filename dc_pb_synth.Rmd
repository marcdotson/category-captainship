...
Store 1782306 is a serious problem, only 40 dates. Store codes above that are fine. Store codes below have not been checked. 

```{r}
week_vec <- unique(dc_data$week_end)

single_store <- dc_data %>% filter(store_code_uc==1782306)
...
dc_data <- dc_data %>%
  ...
  mutate(store_code_uc=as.numeric(store_code_uc), week_end=as.numeric(week_end))
