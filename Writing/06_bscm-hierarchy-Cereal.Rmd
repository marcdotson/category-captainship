...
pl_treat_sales <- pl_treat_sales %>% filter(store_num %in% c(1:5))
```
...
#create Z KxN matrix 
tZ <- pl_treat %>% filter(store_num %in% c(1:5)) %>% dplyr::select(store_num, price_diff, num_upcs) %>% distinct() %>% dplyr::select(price_diff, num_upcs) %>% as.matrix()
Z <- t(tZ)
Z <- Z %>% rbind(rep(1, N))
K <- nrow(Z)
