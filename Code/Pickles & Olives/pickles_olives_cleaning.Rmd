---
title: "Pickles and Olives Data Cleaning"
output: github_document
---

This code imports, cleans, and saves the data for the pickles and olives category in the four locations of interest.
...
pickles_files <- untar("Pickles_Olives.tgz", list=TRUE)

move_11 <- tibble()
for (i in seq_along(pickles_files)) {
  
  if (substr(pickles_files[i], 22, 40) == "2011/Movement_Files") {
    
    temp_move_11 <- read_tsv(pickles_files[i])
...
for (i in seq_along(pickles_files)) {
  ...
  if (substr(pickles_files[i], 22, 40) == "2012/Movement_Files") {
    
    temp_move_12 <- read_tsv(pickles_files[i])
...
for (i in seq_along(pickles_files)) {
  ...
  if (substr(pickles_files[i], 22, 40) == "2013/Movement_Files") {
    
    temp_move_13 <- read_tsv(pickles_files[i])
...
Read in file containing the manufacturer upc prefixes. The captain and validator for cereal are Pinnacle and
Musco, respectively. Create separate vectors with the upc prefixes for both Pinnacle and Musco called
`pinnacle_prefs` and `musco_prefs`.
```{r}
prefixes <- read_xlsx('Manufacturer UPC Prefixes - From GS1 Company Database.xlsx')

pinnacle_prefs <- prefixes %>%
  filter(manufacturer == 'Pinnacle') %>%
  select(prefix) %>%
  pull()

musco_prefs <- prefixes %>%
  filter(manufacturer == "Musco") %>%
  select(prefix) %>%
  pull()
```

Create `full_pickles` by combining the data for 2011 and 2012. Convert `week_end` to be a date variable and create a
...
```{r}
full_pickles <- full_11 %>%
  bind_rows(full_12) %>% bind_rows(full_13) %>% 
  mutate(week_end = ymd(week_end),
         sales = units * price,
         manufacturer = if_else(grepl(paste(pinnacle_prefs, collapse = '|'), upc), 'Pinnacle',
                        if_else(grepl(paste(musco_prefs, collapse = '|'), upc), "Musco",
...
`missing_stores`.
```{r}
missing_stores <- full_pickles %>%
  distinct(store_code_uc, panel_year) %>%
  group_by(store_code_uc) %>%
  summarize(n_years = n()) %>%
  filter(n_years < length(unique(year(full_pickles$week_end)))) %>%
...
```{r}
retailer_switchers <- full_pickles %>%
...
include an `NA` value. Filter `full_pickles` for the upc codes that have multiple valid brand descriptions and compare
...
```{r}
multi_brand_descr <- full_pickles %>%
...
upc had zero sales. First, create `full_weeks` which contains all values of `week_end`. Split `full_pickles` into
...
to the original data. Recombine the separate `full_pickles` data into one `tibble`.
```{r}
full_weeks <- full_pickles %>%
  distinct(week_end)

full_pickles <- full_pickles %>%
  group_split(store_code_uc, upc)

for (i in 1:length(full_pickles)) {
  
  if(nrow(full_pickles[[i]]) < length(full_weeks$week_end)) {
  
  vars_vec <- full_pickles[[i]][1,]
  
  missing_weeks <- full_weeks %>%
      anti_join(full_pickles[[i]], by = 'week_end') %>%
      pull()
  ...
  full_pickles[[i]] <- full_pickles[[i]] %>%
    bind_rows(missing_data)
  
  }}

full_pickles <- bind_rows(full_pickles)
```
...
```{r}
save(full_pickles, file = 'pickles_clean.RData')

mn_pickles <- full_pickles %>% filter(fips_state_descr == 'MN')
il_pickles <- full_pickles %>% filter(fips_state_descr == 'IL')
mo_pickles <- full_pickles %>% filter(fips_state_descr == 'MO')
nd_pickles <- full_pickles %>% filter(fips_state_descr == 'ND')

save(mn_pickles, file="mn_pickles_clean.RData")
save(il_pickles, file="il_pickles_clean.RData")
save(mo_pickles, file="mo_pickles_clean.RData")
save(nd_pickles, file="nd_pickles_clean.RData")
```
