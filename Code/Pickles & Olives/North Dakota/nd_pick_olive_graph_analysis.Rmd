---
title: "North Dakota Pickles/Olives Graph Analysis"
...
```
Load North Dakota data.
```{r}
load('nd_pickles_olives_clean.RData')
```
Create `tbl`, `main_manuf_sales` which contains movement data for the brands owned by Musco, Pinnacle, and private label. Create a new variable `actual_manuf` which denotes ownership by either pinnacle, private label, or musco. This allows for sales across different brands to be aggregated later to sales by manufacturer.
```{r}
...
```
Create `retailer_weekly` which is filtered for retailers in North Dakota, grouped by retailer code, brand_name, and week_end, creating weekly sales nd average prices by manufacturer within retailer. `ret_codes` contains each retailer code. The 'for' loop goes over each unique retailer code and graphs the weekly sales by manufacturer for each retailer code. It also plots the average weekly price by manufacturer by retailer. It places these graphs next to each other for easy comparison. There is a verticle line at the approximate CC implementation date. Each graph includes year specific regression lines.
```{r}
