---
title: "Minnesota Pickles/Olives Retailer Test"
author: "Cameron Bale"
date: "5/14/2019"
...
Assign the manfacturer (Private Label, Musco, Pinnacle) for each brand. Convert store code to be a numeric. Organize data by retailer, stores and date, then sum sales across UPCs, giving us weekly sales for each manufacturer in each
...
mn_data <- mn_data %>% 
  mutate(manuf=if_else(actual_manuf=="CTL", 1, 
               if_else(actual_manuf=="MUSCO", 2, 3)))
```
...
Create a vector of the unique retailer codes in MN Data. Create a loop that creates a synthetic control for each manufacturer in each store for each retailer. Collect the p values and treatment effects from each synthetic control. The target reset date was 10/08/2012, we chose the implementation date for the week after (since 10/08 isn't an option), which was 10/13/2012.
...
msynth_results %>%
  ...
  facet_wrap(~retailer) + labs(x="P-Value", y="Count", title="Manufacturer Level P-Values for MN Pickles/Olives")
```
...
msynth_results %>%
  ...
  geom_histogram() +
  geom_vline(xintercept = 0, col = 'blue') +
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Manufacturer Level Treatment Effects for MN Pickles/Olives")
```
...
msynth_results %>%
  ...
  geom_histogram() +
  geom_vline(xintercept = 0, col = 'blue') +
  facet_wrap(~retailer) + labs(x="P-Value", y="Count", title="Store Level P-Values for Retailers in MN Pickles/Olives")
```
...
msynth_results %>%
  ...
  geom_histogram() +
  geom_vline(xintercept = 0, col = 'blue') +
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Store Level Treatment Effects for MN Pickles/Olives")
```
...
msynth_results %>%
  ...
  labs(x="Treatment Effect", 
       y="Count", 
       title="Store Level Treatment Effects for MN Pickles/Olives",
...
msynth_results %>%
  ...
  labs(x = 'P-Value',
       y = 'Count',
       title = 'P-Values for Retailer Combinations at the Store Level: MN Pickles/Olives')
```
...
msynth_results %>%
  ...
  labs(x = 'Effect',
       y = 'Count',
       title = 'Treatment Effects for Retailer Combinations at Store Level: MN Pickles/Olives')
```
