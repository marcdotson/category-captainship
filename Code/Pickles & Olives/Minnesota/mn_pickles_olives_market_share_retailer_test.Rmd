---
title: "Minnesota Pickles and Olives Market Share Retailer Test"
...
Create `total_sales` which contains total weekly sales by store. In `mn_data`, create 
weekly sales by manufacturer by store, join with `total_sales`, and create the variable
`market_share` by dividing weekly manufacturer sales by weekly total store sales. 
```{r}
total_sales <- mn_data %>%
...
Assign the manfacturer (Private Label, Pinnacle) for each brand. Convert store code to be a numeric.
```{r}
mn_data <- mn_data %>%
  filter(manuf_name %in% c("VLA", "NAL", "FAR", "CTL")) %>%
  mutate(actual_manuf = if_else(manuf_name == "CTL", "private_label", 'pinnacle')) %>%
...
msynth_results %>%
  ...
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Manufacturer Level Significant Treatment Effects: MN Pickles and Olives", subtitle = "Market Share")
```
...
msynth_results %>%
  ...
  labs(x = 'Effect',
       y = 'Count',
       title = 'Significant Treatment Effects for Retailer Combinations at Manufacturer Level: MN Pickles and Olives',
