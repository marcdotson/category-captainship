---
title: "Missouri Ice Cream Market Share Retailer Test"
...
```{r}
mo_data <- mo_data %>%
  filter(manuf_name %in% c("BEN", "TAL", "BRE", "UNI", "CTL")) %>%
  mutate(actual_manuf = if_else(manuf_name=="BEN", "unilever", if_else(manuf_name=="TAL", "unilever", if_else(manuf_name=="BRE", "unilever", if_else(manuf_name=="UNI", "unilever", "private_label"))))) %>%
...
msynth_results %>%
  ...
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Manufacturer Level Significant Treatment Effects: MO Ice Cream", subtitle = "Market Share")
```
...
Create `ret_code_combo` that pairs each retailer with each other retailer, giving each the possibility to be the treated retailer code. Remove the cases where the control retailer only has one store (none here).
```{r}
...
msynth_results %>%
  ...
  labs(x = 'Effect',
       y = 'Count',
       title = 'Significant Treatment Effects for Retailer Combinations at Manufacturer Level: MO Ice Cream',
