...
Assign the manfacturer (Private Label, Unilever, Nestle) for each brand. Convert store code to be a numeric. Organize data by retailer, stores and date, then sum sales across UPCs, giving us weekly sales for each manufacturer in each
...
msynth_results %>%
  ...
  geom_histogram() +
  geom_vline(xintercept = 0, col = 'blue') +
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Store Level Treatment Effects for MN Ice Cream", subtitle = 'Statistically Significant Effects')
```
