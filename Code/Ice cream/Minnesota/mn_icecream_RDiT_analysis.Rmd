---
title: "Minnesota Cereal Regression Discontinuity Analysis"
...
Load data.
```{r}
load(here::here("..", "mn_ice_cream_clean.RData"))
```
...
for (i in seq_along(ret_codes)) {
  ...
  captain_prods <- mn_data %>%
    filter(retailer_code == ret_codes[i] & manuf_name %in% c("BEN", "TAL", "BRE", "UNI")) %>%
  ...
  mkt_share_plot <- mkt_share %>%
    ggplot(aes(x = week_end, 
               y = capt_market_share)) +
    geom_point() +
    geom_vline(xintercept = as.numeric(as.Date("2012-03-31")), 
               color = 'red') +
    geom_vline(xintercept = as.numeric((as.Date("2013-03-30")))) +
