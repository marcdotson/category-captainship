---
title: "Minnesota Ice Cream Price Retailer Test "
author: "Morgan Bale"
date: "May 20, 2019"
  ...
  for (j in seq_along(t_store_codes)) {
    ...
    for (k in seq_along(manufs)) {
      ...
      msynth <- microsynth(data = as.data.frame(manuf_data),
                           ...
                           match.out = "avg_price",
                           match.covar = "avg_price",
                           result.var="avg_price", 
                           omnibus.var="avg_price",
                           test="twosided", 
                           use.survey = FALSE)
...
msynth_results %>%
  ...
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Manufacturer Level Significant Treatment Effects: MN Ice Cream", subtitle = "Average Price")
```

Average price across the captain and private label. 
```{r, message=FALSE, warning=FALSE}
mn_store_data <- mn_data %>% group_by(retailer_code, store_code_uc, n_week_end) %>% summarise(avg_price=mean(avg_price))
```
...
msynth_results %>%
  ...
  labs(x = 'Effect',
       y = 'Count',
       title = 'Significant Treatment Effects for Retailer Combinations at Manufacturer: MN Ice Cream',
