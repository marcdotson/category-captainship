...
generated quantities {
  ...
  real<lower=0> nu; //standard deviation in SC equation 
  ...
  tau=normal_rng(0,5);
  alpha=normal_rng(0,5);
  gamma=normal_rng(0,5);
  sigma=normal_rng(0,5);
  nu=normal_rng(0,5);
  eta=normal_rng(0,5);
  
  for(c in 1:C) {w[c] = normal_rng(0, .5);} 
  
  //sc equation for weights
  for(i in 1:I) {
    for(c in 1:C) {
      Y_treat_pre[i]=normal_rng(eta + w[c]*Y_con_pre[c, i], nu);
    }
  }
  //DiD equation 
  for(n in 1:N) {
    for(t in 1:TT) {
    Y[n, t]=normal_rng(mu + alpha*treat[n] + gamma*post[t] + tau*D[n,t], sigma*omega[n]);
    }
  }
}
