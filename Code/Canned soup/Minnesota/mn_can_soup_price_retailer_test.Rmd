---
title: "Minnesota Canned Soup Price Retailer Test"
...
Assign the manfacturer (Private Label, Campbells) for each brand. Convert store code to be a numeric. Organize data by retailer, stores and date, then sum sales across UPCs, giving us weekly sales for each manufacturer in each store in each retailer.
```{r}
mn_data <- mn_data %>%
  filter(manuf_name %in% c("CAM", "CTL")) %>%
  mutate(actual_manuf = if_else(manuf_name == "CAM", "campbells", 'private_label')) %>%
...
msynth_results %>%
  ...
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Manufacturer Level Significant Treatment Effects: MN Canned Soup", subtitle = "Average Price")
```
...
msynth_results %>%
  ...
  labs(x = 'Effect',
       y = 'Count',
       title = 'Significant Treatment Effects for Retailer Combinations at Store Level: MN Canned Soup',
