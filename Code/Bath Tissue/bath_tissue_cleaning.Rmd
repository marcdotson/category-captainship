---
title: "Bath Tissue Cleaning"
output: github_document
---

Note: This document contains all the code used to merge and clean data for the bath tissue category. 
...
Untar (unzip) the `.tgz` file. Read in 2012 movement, stores, rms, and the products master file.
```{r}
bt_files <- untar('BathTissue_2012-14.tgz', list = TRUE)

untar('BathTissue_2012-14.tgz')

move_12 <- read_tsv("nielsen_extracts/RMS/2012/Movement_Files/4507_2012/7260_2012.tsv")
...
Create `full_12` by joining the 2012 stores file to the movement file on `store_code_uc`. Then filter for `F` (food)
and `ND` (North Dakota) `MN` (Minnesota) `MO` (Missouri) and `IL` (Illinois). Join the 2012 rms file to the movement
...
rm(move_12, rms_12, stores_12)
```

Repeat for 2013.
```{r}
move_13 <- read_tsv("nielsen_extracts/RMS/2013/Movement_Files/4507_2013/7260_2013.tsv")
...
rm(stores_13, rms_13, move_13)
```

Repeat for 2014.
```{r}
move_14 <- read_tsv("nielsen_extracts/RMS/2014/Movement_Files/4507_2014/7260_2014.tsv")
...
rm(products, move_14, stores_14, rms_14, bt_files)
```

Read in file containing the manufacturer upc prefixes. The captain and validator for bath tissue are Kimberly-Clark and
Georgia-Pacific, respectively. Create separate vectors with the upc prefixes for both Kimberly-Clark and Georgia-Pacific called
`KC_prefs` and `GP_prefs`.
```{r}
prefixes <- read_xlsx('Manufacturer UPC Prefixes - From GS1 Company Database.xlsx')

KC_prefs <- prefixes %>%
  filter(manufacturer == 'Kimberly-Clark') %>%
  select(prefix) %>%
  pull()

GP_prefs <- prefixes %>%
  filter(manufacturer == 'Georgia-Pacific') %>%
  select(prefix) %>%
  pull()
```

Create `full_bt` by combining the data for 2012-2014. Convert `week_end` to be a date variable. Create `sales` by multiplying `units` and
`price`. Create `manufacturer` by assinging the correct manufacturer based on upc prefixes. We assign Kimberly-Clark, Georgia-Pacific,
...
full_bt <- full_12 %>%
  ...
  mutate(week_end = ymd(week_end),
         sales = units * price,
         manufacturer = if_else(grepl(paste(KC_prefs, collapse = '|'), upc), 'Kimberly-Clark',
                        if_else(grepl(paste(GP_prefs, collapse = '|'), upc), 'Georgia-Pacific',
                        if_else(brand_descr == 'CTL BR', 'Private Label', 'Other', missing = 'Other'))))

rm(full_12, full_13, full_14, prefixes, GP_prefs, KC_prefs)
```
...
`missing_stores`.
```{r}
missing_stores <- full_bt %>%
  distinct(store_code_uc, panel_year) %>%
  group_by(store_code_uc) %>%
  summarize(n_years = n()) %>%
  filter(n_years < length(unique(year(full_bt$week_end)))) %>%
...
include an `NA` value. Filter `full_bt` for the upc codes that have multiple valid brand descriptions and compare
...
Check if there are private label products with `NA` brand description values, which we should change to `'CTL BR'` so that they are
aggregated under the correct manufacturer. Change `NA` brand descriptions for private label products. 
```{r}
full_bt %>%
  filter(upc %in% multi_brand_descr) %>%
  distinct(upc, brand_descr) %>%
...
# no private label products have NA brand description values so we can move on
#priv_lab_na <- full_bt %>%
#  filter(upc %in% multi_brand_descr) %>%
#  distinct(upc, brand_descr) %>%
...
upc had zero sales. First, create `full_weeks` which contains all values of `week_end`. Split `full_bt` into
...
to the original data. Recombine the separate `full_bt` data into one `tibble`.
```{r}
full_weeks <- full_bt %>%
  distinct(week_end)
...
for (i in 1:length(full_bt)) {
  
  if(nrow(full_bt[[i]]) < length(full_weeks$week_end)) {
  
  vars_vec <- full_bt[[i]][1,]
  
  missing_weeks <- full_weeks %>%
      anti_join(full_bt[[i]], by = 'week_end') %>%
      pull()
...
```{r}
save(full_bt, file = 'bath_tissue_clean.RData')

mn_bt <- full_bt %>% filter(fips_state_descr == 'MN')
il_bt <- full_bt %>% filter(fips_state_descr == 'IL')
mo_bt <- full_bt %>% filter(fips_state_descr == 'MO')
nd_bt <- full_bt %>% filter(fips_state_descr == 'ND')

save(mn_bt, file="mn_bath_tissue_clean.RData")
save(il_bt, file="il_bath_tissue_clean.RData")
save(mo_bt, file="mo_bath_tissue_clean.RData")
save(nd_bt, file="nd_bath_tissue_clean.RData")
```