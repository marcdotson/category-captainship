...
Load 'min_pb_data.RData' file.
```{r}
load("min_pb_data.RData")

# in this case, we don't know which store is treated
...
```{r}
min_pb_data <- min_pb_data %>%
  mutate(manuf_name = substr(min_pb_data$brand_descr, 1, 3)) %>%
...
min_pb_data <- min_pb_data %>% group_by(retailer_code, store_code_uc, actual_manuf, week_end) %>% summarize(sales=sum(sales)) %>%
...
```{r}
weeks <- unique(min_pb_data$week_end)

codes <- unique(min_pb_data$store_code_uc)

drops <- c()

for (i in seq_along(codes)) {
  
  single_store <- min_pb_data %>% filter(store_code_uc==codes[i])
...
min_pb_data <- min_pb_data %>% filter(!store_code_uc %in% drops) %>% arrange(retailer_code, store_code_uc, week_end) %>%
...
min_pb_data <- min_pb_data %>% mutate(manuf=if_else(actual_manuf=="CTL", 1, if_else(actual_manuf=="SMU", 2, 3)))
```
...
ret_codes <- unique(min_pb_data$retailer_code)
msynth_results <- tibble()

for (i in seq_along(ret_codes)) {
  msynth_pvals <- c()
  msynth_effects <- c()
  t_stores <- min_pb_data %>% filter(retailer_code==ret_codes[i]) 
  c_stores <- min_pb_data %>% filter(retailer_code!=ret_codes[i]) %>% mutate(type=0) 
  ...
  for (j in seq_along(t_store_codes)) {
    ...
    manufs <- unique(min_pb_data$manuf)
...
```{r, message=FALSE, warning=FALSE}
dc_store_data <- min_pb_data %>% group_by(retailer_code, store_code_uc, n_week_end) %>% summarise(sales=sum(sales))
