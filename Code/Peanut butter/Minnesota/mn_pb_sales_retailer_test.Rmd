---
title: "Minnesota Peanut Butter Retailer Test"
...
Load in the data for Minnesota Peanut Butter category.  
```{r}
load("../../../../mn_pb_clean.RData")
```

Assign the manfacturer (Private Label, Unilever, Smuckers) for each brand. Convert store code to be a numeric. Organize data by retailer, stores and date, then sum sales across UPCs, giving us weekly sales for each manufacturer in each store in each retailer.
```{r}
mn_data <- mn_data %>%
...
msynth_results %>%
  ...
  facet_wrap(~retailer) + labs(x="P-Value", y="Count", title="Manufacturer Level P-Values for MN Peanut Butter")
```
...
msynth_results %>%
  ...
  geom_histogram() +
  geom_vline(xintercept = 0, col = 'blue') +
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Manufacturer Level Treatment Effects for MN Peanut Butter")
```
...
msynth_results %>%
  ...
  geom_histogram() +
  geom_vline(xintercept = 0, col = 'blue') +
  facet_wrap(~retailer) + labs(x="P-Value", y="Count", title="Store Level P-Values for Retailers: MN Peanut Butter")
```
...
msynth_results %>%
  ...
  geom_histogram() +
  geom_vline(xintercept = 0, col = 'blue') +
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Store Level Treatment Effects for MN Peanut Butter")
```
...
msynth_results %>%
  ...
  geom_histogram() +
  geom_vline(xintercept = 0, col = 'blue') +
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Store Level Treatment Effects: MN Peanut Butter", subtitle = "Statstically Significant Effects")
```
...
for (i in 1:nrow(ret_code_combo)) {
  ...
  for (j in seq_along(t_store_codes)) {
    store <- t_stores %>% filter(store_code_uc==t_store_codes[j]) %>% 
      mutate(type = ifelse(n_week_end > 15542, 1, 0))
    store_data <- bind_rows(store, c_stores)
         msynth <- microsynth(data = as.data.frame(store_data),
                           idvar = "store_code_uc",
                           intvar = "type",
...
msynth_results %>%
  ...
  labs(x = 'P-Value',
       y = 'Count',
       title = 'P-Values for Retailer Combinations at the Store Level: MN Peanut Butter')
```
...
msynth_results %>%
  ...
  labs(x = 'Effect',
       y = 'Count',
       title = 'Treatment Effects for Retailer Combinations at Store Level: Peanut Butter')
```
...
msynth_results %>%
  ...
  labs(x = 'Effect',
       y = 'Count',
       title = 'Treatment Effects for Retailer Combinations at Store Level: MN Peanut Butter',
