---
title: "Missouri Peanut Butter Sales Retailer Test"
output: github_document
---

Load packages and upload the data. 
```{r}
library(tidyverse)
library(microsynth)
library(lubridate)
...
To analyze at the manufacturer level, we need to remove manufacturers that don't have sales in the full time period because it causes an error in Microsynth. This code loops over every store, and for every store it loops over each manufacturer and removes that manufacturer in that store if it doesn't have sales for the full time period. Create a vector of the unique retailer codes in MO Data. The target reset date was 07/16/2012, we chose the implementation date for the week after (since 07/16 isn't an option), which was 07/21/2012. Aggregate sales across stores.
```{r}
...
Check for retailers that only have one store, as this produces as error in Microsynth. Create `ret_code_combo` that pairs each retailer with each other retailer, giving each the possibility to be the treated retailer code. Use `mo_store_data` which is aggregated across stores.
```{r}
...
msynth_results %>%
  ...
  labs(x = 'Effect',
       y = 'Count',
       title = 'Treatment Effects for Retailer Combinations at Store Level: MO Peanut Butter',
