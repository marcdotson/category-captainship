---
title: "Cheese Cleaning File"
author: "Cameron Bale"
date: "9/10/2019"
output: html_document
---

Note: This document contains all the code used to merge and clean data for the cheese category. 
...
```{r}
cheese_files <- untar('cheese_2011-13.tgz', list = TRUE)

untar('cheese_2011-13.tgz')

move_11 <- tibble()

for (i in seq_along(cheese_files)) {
  
  if (substr(cheese_files[i], 22, 40) == "2011/Movement_Files") {
    
    temp_move_11 <- read_tsv(cheese_files[i])
...
for (i in seq_along(cheese_files)) {
  ...
  if (substr(cheese_files[i], 22, 40) == "2012/Movement_Files") {
    
    temp_move_12 <- read_tsv(cheese_files[i])
...
for (i in seq_along(cheese_files)) {
  ...
  if (substr(cheese_files[i], 22, 40) == "2013/Movement_Files") {
    
    temp_move_13 <- read_tsv(cheese_files[i])
...
Read in file containing the manufacturer upc prefixes. The captain and validator for cheese are Kraft and
Sargento, respectively. Create separate vectors with the upc prefixes for both Kraft and Sargento called
`kraft_prefs` and `sarg_prefs`.
```{r}
...
sarg_prefs <- prefixes %>%
  filter(manufacturer == 'Sargento') %>%
  select(prefix) %>%
  pull()
```

Create `full_cheese` by combining the data for 2011 and 2012. Convert `week_end` to be a date variable and create a
numeric version called `n_week_end` which is needed for microsynth. Create `sales` by multiplying `units` and `price`.
Create `manufacturer` by assinging the correct manufacturer based on upc prefixes. We assign Kraft, Sargento,
...
full_cheese <- full_11 %>%
  ...
  mutate(week_end = ymd(week_end),
         sales = units * price,
         manufacturer = if_else(grepl(paste(kraft_prefs, collapse = '|'), upc), 'Kraft',
                        if_else(grepl(paste(sarg_prefs, collapse = '|'), upc), 'Sargento',
...
`missing_stores`.
```{r}
missing_stores <- full_cheese %>%
  distinct(store_code_uc, panel_year) %>%
  group_by(store_code_uc) %>%
  summarize(n_years = n()) %>%
  filter(n_years < length(unique(year(full_cheese$week_end)))) %>%
...
```{r}
retailer_switchers <- full_cheese %>%
...
include an `NA` value. Filter `full_cheese` for the upc codes that have multiple valid brand descriptions and compare
...
upc had zero sales. First, create `full_weeks` which contains all values of `week_end`. Split `full_cheese` into
...
to the original data. Recombine the separate `full_cheese` data into one `tibble`.
```{r}
full_weeks <- full_cheese %>%
  distinct(week_end)

full_cheese <- full_cheese %>%
  group_split(store_code_uc, upc)

for (i in 1:length(full_cheese)) {
  
  if(nrow(full_cheese[[i]]) < length(full_weeks$week_end)) {
  
  vars_vec <- full_cheese[[i]][1,]
  
  missing_weeks <- full_weeks %>%
      anti_join(full_cheese[[i]], by = 'week_end') %>%
      pull()
  ...
  full_cheese[[i]] <- full_cheese[[i]] %>%
    bind_rows(missing_data)
  
  }}

full_cheese <- bind_rows(full_cheese)
```
...
```{r}
save(full_cheese, file = 'cheese_clean.RData')

mn_cheese <- full_cheese %>% filter(fips_state_descr == 'MN')
il_cheese <- full_cheese %>% filter(fips_state_descr == 'IL')
mo_cheese <- full_cheese %>% filter(fips_state_descr == 'MO')
nd_cheese <- full_cheese %>% filter(fips_state_descr == 'ND')

save(mn_cheese, file="mn_cheese_clean.RData")
save(il_cheese, file="il_cheese_clean.RData")
save(mo_cheese, file="mo_cheese_clean.RData")
save(nd_cheese, file="nd_cheese_clean.RData")
```