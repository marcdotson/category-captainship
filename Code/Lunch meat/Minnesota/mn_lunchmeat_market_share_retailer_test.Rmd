---
title: "Minnesota Lunchmeat Market Share Retailer Test"
author: "Cameron Bale"
date: "6/8/2019"
...
```{r}
load(here::here("..", "mn_lunchmeat_clean.RData"))
```

Create `total_sales` which is total weekly sales in each store. Use the `group_by` and 
`summarize` functions on `mn_data` to create total weekly sales for each manufacturer in each
store. Join the new `mn_data` to `total_sales` and create `market_share` by dividing each
manufacturer's weekly sales by the total weekly sales in each store.
```{r}
total_sales <- mn_data %>%
...
Assign the manfacturer (Private Label, Oscar Mayer) for each brand. Convert store code to be a numeric. Organize data by retailer, stores and date, then sum sales across UPCs, giving us weekly sales for each manufacturer in each store in each retailer.
```{r}
mn_data <- mn_data %>%
  filter(manuf_name %in% c("OSC", "CTL")) %>%
  mutate(actual_manuf = if_else(manuf_name == "OSC", "oscar_mayer", "private_label")) %>%
...
msynth_results %>%
  ...
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Manufacturer Level Significant Treatment Effects: MN Lunchmeat", subtitle = "Market Share")
```
...
msynth_results %>%
  ...
  labs(x = 'Effect',
       y = 'Count',
       title = 'Significant Treatment Effects for Retailer Combinations at Manufacturer Level: MN Lunchmeat',
