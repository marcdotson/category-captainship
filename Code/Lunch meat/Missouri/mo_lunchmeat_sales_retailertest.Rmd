---
title: "Missouri Lunchmeat Sales Retailer Test"
output: github_document
---

Load packages and install the data. 
```{r}
library(tidyverse)
library(microsynth)
library(lubridate)
...
Assign the manfacturer (Private Label, Oscar Mayer, Sara Lee) for each brand. Convert store code to be a numeric. Organize data by retailer, stores and date, then sum sales across UPCs, giving us weekly sales for each manufacturer in each store in each retailer.
```{r}
mo_data <- mo_data %>%
...
mo_data <- mo_data %>% 
  mutate(manuf=if_else(actual_manuf=="CTL", 1, 
               if_else(actual_manuf=="SARALEE", 2, 3)))
```
...
Create a vector of the unique retailer codes in MO Data. The target reset date was 07/16/2012, we chose the implementation date for the week after (since 07/16 isn't an option), which was 07/21/2012. Aggregate sales across stores. Check for retailers that only have one store, as this produces as error in Microsynth. Create `ret_code_combo` that pairs each retailer with each other retailer, giving each the possibility to be the treated retailer code. Use `mo_store_data` which is aggregated across stores.
```{r}
...
msynth_results %>%
  ...
  labs(x = 'Effect',
       y = 'Count',
       title = 'Treatment Effects for Retailer Combinations at Store Level: MO Lunch Meat',
