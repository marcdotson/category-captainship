...
Filter for North Dakota data and impute zero sales. 
```{r}
nd_cereal <- full_cereal %>% filter(fips_state_descr == 'ND')

stores <- unique(nd_cereal$store_code_uc)

full_weeks <- nd_cereal %>% distinct(week_end) 

for (store in seq_along(stores)) {
  
  single_store <- nd_cereal %>%
    filter(store_code_uc == stores[store])
...
Filter for Minnesota and impute zero sales. 
```{r}
mn_cereal <- full_cereal %>% filter(fips_state_descr == 'MN')

stores <- unique(mn_cereal$store_code_uc)

full_weeks <- mn_cereal %>% distinct(week_end) 

for (store in seq_along(stores)) {
  
  single_store <- mn_cereal %>%
    filter(store_code_uc == stores[store])
...
Filter for Missouri and impute zero sales
```{r}
mo_cereal <- full_cereal %>% filter(fips_state_descr == 'MO')

stores <- unique(mo_cereal$store_code_uc)

full_weeks <- mo_cereal %>% distinct(week_end) 

for (store in seq_along(stores)) {
  
  single_store <- mo_cereal %>%
    filter(store_code_uc == stores[store])
...
Filter for Illinios and impute zero sales
```{r}
il_cereal <- full_cereal %>% filter(fips_state_descr == 'IL')

stores <- unique(il_cereal$store_code_uc)

full_weeks <- il_cereal %>% distinct(week_end) 

for (store in seq_along(stores)) {
  
  single_store <- il_cereal %>%
    filter(store_code_uc == stores[store])
