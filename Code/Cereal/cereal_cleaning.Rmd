...
Read in files for 2010 and create `full_10` the same way as `full_11`.
```{r}
move_10 <- read_tsv("1344_2010.tsv")
stores_10 <- read_tsv("stores_2010.tsv")
rms_10 <- read_tsv("rms_versions_2010.tsv")

full_10 <- move_10 %>%
  left_join(stores_10, by = "store_code_uc") %>% 
  filter(channel_code == "F" & fips_state_descr %in% c("ND", "MN", "MO" ,"IL")) %>%
  left_join(rms_10, by = 'upc') %>%
  left_join(products, by = c('upc', 'upc_ver_uc'))

rm(move_10, rms_10, stores_10)
```
...
Combine all four years into one file, `full_data`. Change `week_end` variable to year-month-day format. Create
