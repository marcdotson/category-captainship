...
Untar (unzip) the `.tgz` file for cereal. Read in 2011 movement, stores, rms, and the products master file.
```{r}
...
missing_stores <- full_cereal %>%
  ...
  summarize(n_years = n()) %>%
  filter(n_years < length(unique(year(full_cereal$week_end)))) %>%
