...
Label brands with their actual manufacturer. Create a heat map chart that shows all UPCs in store 2314183 in retailer 221, with unit sales in each cell. Filter for the store we want. 
```{r}
mn_data <- mn_data %>% mutate(actual_manuf = if_else(manuf_name == "G M", "G M", if_else(manuf_name == "ANN", "G M", if_else(manuf_name == "CAS", "G M", 
                        if_else(manuf_name == "KEL", "KEL", if_else(manuf_name == "KAS", "KEL", if_else(manuf_name == "BEA", "KEL", if_else(manuf_name=="CTL", "CTL", "other"))))))))

descr <- mn_data %>% filter(store_code_uc== 2314183) %>% distinct(upc, brand_descr, actual_manuf)
...
chart <- chart %>% left_join(descr, by="upc") 

chart_1 <- chart %>% spread(key=week_end, value=units)
...
csv_chart <- formattable(chart_1, lapply(1:nrow(chart_1), function(row) {area(row, col= -1:-3) ~ color_tile(customGreen0, customGreen) } ) ) 

write.csv(csv_chart, file="heatmap1.csv")
```
