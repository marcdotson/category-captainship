---
title: "Coffee Data Cleaning"
output: github_document
---

Note: This document contains all the code used to merge and clean data for the coffee category. 
...
```{r}
coffee_files <- untar('Coffee_2011-13.tgz', list = TRUE)
untar('Coffee_2011-13.tgz')

move_11 <- tibble()

for (i in seq_along(coffee_files)) {
  
  if (substr(coffee_files[i], 22, 40) == "2011/Movement_Files") {
    
    temp_move_11 <- read_tsv(coffee_files[i])
...
for (i in seq_along(coffee_files)) {
  ...
  if (substr(coffee_files[i], 22, 40) == "2012/Movement_Files") {
    
    temp_move_12 <- read_tsv(coffee_files[i])
...
for (i in seq_along(coffee_files)) {
  ...
  if (substr(coffee_files[i], 22, 40) == "2013/Movement_Files") {
    
    temp_move_13 <- read_tsv(coffee_files[i])
...
Read in file containing the manufacturer upc prefixes. The captain and validator for cereal are Smucker's and
Kraft, respectively. Create separate vectors with the upc prefixes for both Smucker's and Kraft called
`kraft_prefs` and `smuckers_prefs`.
```{r}
...
Create `full_coffee` by combining the data for 2011 and 2012. Convert `week_end` to be a date variable and create a
...
full_coffee <- full_11 %>%
  ...
  mutate(week_end = ymd(week_end),
         ...
         manufacturer = if_else(grepl(paste(smuckers_prefs, collapse = '|'), upc), 'Smuckers',
                        if_else(grepl(paste(kraft_prefs, collapse = '|'), upc), "Kraft",
...
`missing_stores`.
```{r}
missing_stores <- full_coffee %>%
  distinct(store_code_uc, panel_year) %>%
  group_by(store_code_uc) %>%
  summarize(n_years = n()) %>%
  filter(n_years < length(unique(year(full_coffee$week_end) %>%
...
```{r}
retailer_switchers <- full_coffee %>%
...
include an `NA` value. Filter `full_coffee` for the upc codes that have multiple valid brand descriptions and compare
...
upc had zero sales. First, create `full_weeks` which contains all values of `week_end`. Split `full_coffee` into
...
to the original data. Recombine the separate `full_coffee` data into one `tibble`.
```{r}
full_weeks <- full_coffee %>%
  distinct(week_end)

full_coffee <- full_coffee %>%
  group_split(store_code_uc, upc)

for (i in 1:length(full_coffee)) {
  
  if(nrow(full_coffee[[i]]) < length(full_weeks$week_end)) {
  
  vars_vec <- full_coffee[[i]][1,]
  
  missing_weeks <- full_weeks %>%
      anti_join(full_coffee[[i]], by = 'week_end') %>%
      pull()
  ...
  full_coffee[[i]] <- full_coffee[[i]] %>%
    bind_rows(missing_data)
  
  }}

full_coffee <- bind_rows(full_coffee)
```
...
```{r}
save(full_coffee, file = 'coffee_clean.RData')

mn_coffee <- full_coffee %>% filter(fips_state_descr == 'MN')
il_coffee <- full_coffee %>% filter(fips_state_descr == 'IL')
mo_coffee <- full_coffee %>% filter(fips_state_descr == 'MO')
nd_coffee <- full_coffee %>% filter(fips_state_descr == 'ND')

save(mn_coffee, file="mn_coffee_clean.RData")
save(il_coffee, file="il_coffee_clean.RData")
save(mo_coffee, file="mo_coffee_clean.RData")
save(nd_coffee, file="nd_coffee_clean.RData")
```
