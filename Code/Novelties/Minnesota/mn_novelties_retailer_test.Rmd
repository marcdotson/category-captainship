---
title: "Minnesota Novelties Retailer Test"
author: "Morgan Bale"
date: "May 10, 2019"
...
Load in the data for Minnesota novelties category.  
```{r}
load("mn_novel_clean.RData")
```

Create `uni_novs` and `nestle_novs` which contain the brands owned by Unilever and Nestle. Create a new variable `actual_manuf` which denotes ownership. This allows for sales across different brands to be aggregated later to sales by manufacturer.Organize data by retailer, stores and date, then sum sales across UPCs, giving us weekly sales for each manufacturer in each store in each retailer.
```{r}
...
msynth_results %>%
  ...
  facet_wrap(~retailer) + labs(x="P-Value", y="Count", title="Manufacturer Level P-Values for MN Novelties")
```
...
msynth_results %>%
  ...
  geom_histogram() +
  geom_vline(xintercept = 0, col = 'blue') +
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Manufacturer Level Treatment Effects for MN Novelties")
```
...
msynth_results %>%
  ...
  geom_histogram() +
  geom_vline(xintercept = 0, col = 'blue') +
  facet_wrap(~retailer) + labs(x="P-Value", y="Count", title="Store Level P-Values for Retailers in MN Novelties")
```
...
msynth_results %>%
  ...
  geom_histogram() +
  geom_vline(xintercept = 0, col = 'blue') +
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Store Level Treatment Effects for MN Novelties")
```
...
msynth_results %>%
  ...
  geom_histogram() +
  geom_vline(xintercept = 0, col = 'blue') +
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Store Level Significant Effects for MN Novelties")
```
...
msynth_results %>%
  ...
  labs(x = 'P-Value',
       y = 'Count',
       title = 'P-Values for Retailer Combinations at the Store Level: MN Novelties')
```
...
msynth_results %>%
  ...
  labs(x = 'Effect',
       y = 'Count',
       title = 'Treatment Effects for Retailer Combinations at Store Level: MN Novelties')
```
