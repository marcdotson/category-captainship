---
title: "Spreads & Jams Data Cleaning"
...
```{r}
sj_files <- untar('Spreads_Jams.tgz', list = TRUE)
untar('Spreads_Jams.tgz')

move_12 <- tibble()

for (i in seq_along(sj_files)) {
  
  if (substr(sj_files[i], 22, 40) == "2012/Movement_Files") {
    
    temp_move_12 <- read_tsv(sj_files[i])
...
for (i in seq_along(sj_files)) {
  ...
  if (substr(sj_files[i], 22, 40) == "2013/Movement_Files") {
    
    temp_move_13 <- read_tsv(sj_files[i])
...
for (i in seq_along(sj_files)) {
  ...
  if (substr(sj_files[i], 22, 40) == "2011/Movement_Files") {
    
    temp_move_11 <- read_tsv(sj_files[i])
...
stores_11 <- read_tsv("nielsen_extracts/RMS/2011/Annual_Files/stores_2011.tsv", guess_max = 20000)
...
rm(move_11, stores_11, rms_11, products)
```

Read in file containing the manufacturer upc prefixes. The captain and validator for spreads and jams are Smucker's and Welch's, respectively. Create separate vectors with the upc prefixes for both Smucker's and Welch's called
`welchs_prefs` and `smuckers_prefs`.
```{r}
...
welchs_prefs <- prefixes %>%
  filter(manufacturer == "Welch's") %>%
  select(prefix) %>%
  pull()
```

Create `full_sj` by combining the data for 2011 and 2012. Convert `week_end` to be a date variable and create a
...
full_sj <- full_11 %>%
  ...
  mutate(week_end = ymd(week_end),
         ...
         manufacturer = if_else(grepl(paste(smuckers_prefs, collapse = '|'), upc), 'Smuckers',
                        if_else(grepl(paste(welchs_prefs, collapse = '|'), upc), "Welchs",
...
`missing_stores`.
```{r}
missing_stores <- full_sj %>%
  distinct(store_code_uc, panel_year) %>%
  group_by(store_code_uc) %>%
  summarize(n_years = n()) %>%
  filter(n_years < length(unique(year(full_sj$week_end)))) %>%
...
include an `NA` value. Filter `full_sj` for the upc codes that have multiple valid brand descriptions and compare
...
upc had zero sales. First, create `full_weeks` which contains all values of `week_end`. Split `full_sj` into
...
to the original data. Recombine the separate `full_sj` data into one `tibble`.
```{r}
full_weeks <- full_sj %>%
  distinct(week_end)
...
for (i in 1:length(full_sj)) {
  
  if(nrow(full_sj[[i]]) < length(full_weeks$week_end)) {
  
  vars_vec <- full_sj[[i]][1,]
  
  missing_weeks <- full_weeks %>%
      anti_join(full_sj[[i]], by = 'week_end') %>%
      pull()
...
```{r}
save(full_sj, file = 'spreads_jams_clean.RData')

mn_sj <- full_sj %>% filter(fips_state_descr == 'MN')
il_sj <- full_sj %>% filter(fips_state_descr == 'IL')
mo_sj <- full_sj %>% filter(fips_state_descr == 'MO')
nd_sj <- full_sj %>% filter(fips_state_descr == 'ND')

save(mn_sj, file="mn_sj_clean.RData")
save(il_sj, file="il_sj_clean.RData")
save(mo_sj, file="mo_sj_clean.RData")
save(nd_sj, file="nd_sj_clean.RData")
```
