---
title: "Missouri Spreads and Jams Market Share Retailer Test"
...
Load in the data for Minnesota Spreads and Jams category.  
```{r}
...
Assign the manfacturer (Private Label, Smuckers) for each brand. Convert store code to be a numeric. Organize data by retailer, stores and date, then sum sales across UPCs, giving us weekly sales for each manufacturer in each store in each retailer.
```{r}
mo_data <- mo_data %>%
  filter(manuf_name %in% c("SMU","SAN","R W", "DIC", "KIN", "CRO", "CTL")) %>% 
  mutate(actual_manuf = if_else(manuf_name == "SMU","SMU",
                        ...
                        if_else(manuf_name == "CRO", "SMU", "CTL"))))))) %>% 
...
msynth_results %>%
  ...
  facet_wrap(~retailer) + labs(x="Treatment Effect", y="Count", title="Manufacturer Level Significant Treatment Effects: MO Spreads and Jams", subtitle = "Market Share")
```
...
msynth_results %>%
  ...
  labs(x = 'Effect',
       y = 'Count',
       title = 'Significant Treatment Effects for Retailer Combinations at Manufacturer Level: MO Spreads and Jams',
