...
captain <- froz_din_slimmer %>% filter(manuf_name %in% c("STO", "LEA", "BUI", "SWE")) %>% group_by(retailer_code, store_code_uc, manuf_name, week_end) %>% summarise(sales=sum(sales))

val <- froz_din_slimmer %>% filter(manuf_name=="CON") %>% group_by(retailer_code, store_code_uc, manuf_name, week_end) %>% summarise(sales=sum(sales))

ctl <- froz_din_slimmer %>% filter(manuf_name=="CTL") %>% group_by(retailer_code, store_code_uc, manuf_name, week_end) %>% summarise(sales=sum(sales))
