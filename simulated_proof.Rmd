...
sim_data <- sim_data %>% group_by(retailer_code, store_code_uc, n_week_end) %>% summarise(sales=sum(sales))
...
for (i in 1:6) {
  ...
  t_stores <- sim_data %>% filter(retailer_code==unname(loop_rets[1])) 
  c_stores <- sim_data %>% filter(retailer_code==unname(loop_rets[2])) %>% mutate(type=0) 
  t_store_codes <- unique(t_stores$store_code_uc)
  for (j in seq_along(t_store_codes)) {
    store <- t_stores %>% filter(store_code_uc==t_store_codes[j]) %>% 
      mutate(type = ifelse(n_week_end > 80, 1, 0))
