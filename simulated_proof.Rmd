...
Try doing a microsynth loop where each retailer is paired with each other retailer. The retailer that causes the pairs to be significant/positive should be the treated retailer. 
```{r}
ret_codes <- unique(sim_data$retailer_code)

ret_code_combo <- as_tibble(expand.grid(treat=ret_codes, control=ret_codes)) %>% filter(treat!=control)

ret_code_combo <- as.matrix(ret_code_combo)
```
...
for (i in 1:6) {
  msynth_pvals <- c()
  msynth_effects <- c()
  loop_rets <- ret_code_combo %>% filter(row_number()==i)
  t_stores <- sim_data %>% filter(retailer_code==unname(as_vector(loop_rets[1]))) 
  c_stores <- sim_data %>% filter(retailer_code==unname(as_vector(loop_rets[2]))) %>% mutate(type=0) 
  t_store_codes <- unique(t_stores$store_code_uc)
  for (j in seq_along(t_store_codes)) {
    store <- t_stores %>% filter(store_code_uc==t_store_codes[j]) %>% 
      mutate(type = ifelse(n_week_end > 80, 1, 0))
