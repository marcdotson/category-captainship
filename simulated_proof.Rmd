...
for (i in ret_codes) {
  ...
  c_stores <- sim_data %>% filter(retailer_code!=ret_codes[i]) 
  t_store_codes <- unique(t_stores$store_code_uc)
  for (j in t_store_codes) {
  store <- t_stores %>% group_by(store_code_uc) %>% filter(store_code_uc==t_store_codes[j]) 
  store_data <- rbind(store, c_stores)
