...
reg_table <- c %>% bind_rows(fd) %>% bind_rows(ic) %>% bind_rows(lm) %>% bind_rows(n) %>% bind_rows(pb) %>% bind_rows(po) %>% bind_rows(sj) %>% bind_rows(sp) %>% mutate(effect=round(effect, digits=3))
